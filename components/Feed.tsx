...
const Feed: React.FC<FeedProps> = ({ lang }) => {
  ...
  return (
    <div className="space-y-6">
      {/* Search Filter */}
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 sticky top-20 z-10">
        <div className="relative">
          ...
          <input 
            ...
            className={`w-full bg-slate-100 rounded-xl py-3 ${lang === 'he' ? 'pr-10 pl-4' : 'pl-10 pr-4'} text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all`}
          />
        </div>
      </div>
      ...
      {filteredPosts.length > 0 ? filteredPosts.map((post) => (
        <article key={post.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100">
          ...
          <div className="px-4 py-3 flex items-center justify-between border-t border-slate-50">
            <div className="flex items-center gap-4">
              <button className="flex items-center gap-1.5 text-slate-500 hover:text-red-500" aria-label={t.likes}>
                <Heart className="w-5 h-5" />
                <span className="text-xs font-medium">{post.likes}</span>
              </button>
              <button className="flex items-center gap-1.5 text-slate-500 hover:text-blue-500" aria-label={t.comments}>
        ...
        <div className="text-center py-12 text-slate-400">
          <p>לא נמצאו כלבים בשם "{searchTerm}"</p>
        </div>
      )}
    </div>
  );
